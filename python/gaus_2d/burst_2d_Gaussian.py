...
def gen_Gauss2D_model(peak_bins, peak_amps, f0=None, bw=None, dt=None, theta=None, verbose=False):
    '''
    ...
    for ii in range(npeaks):
        if ii == 0:
            g_guess = models.Gaussian2D(peak_amps[ii], x_mean=peak_bins[ii], y_mean=f0[ii],
                                        x_stddev=dt[ii], y_stddev=bw[ii], theta=theta[ii])
        else:
            g_guess += models.Gaussian2D(peak_amps[ii], x_mean=peak_bins[ii], y_mean=f0[ii],
                                         x_stddev=dt[ii], y_stddev=bw[ii], theta=theta[ii])
    ...
    if verbose:
        for ii in range(npeaks):
            ...
            print("Amp: %.2f \tTloc: %.2f \tFloc: %.2f \tTwid %.2f \tFwid %.2f Angle: %.2f" % \
                   (peak_amps[ii], peak_bins[ii], f0[ii], dt[ii], bw[ii], theta[ii]))
...
def fit_Gauss2D_model(data, tdum, fdum, g_guess, weights=None):
    ...
    fit_2dG = fit_LM(g_guess, xdum, ydum, data, weights=weights, maxiter=30000)
...
def report_Gauss_parameters(best_gauss, fitter, verbose=False):
    ...
    if cov_mat is None:
        ...
        print("The fit converged after %s iterations"%fitter.fit_info['nfev'])
    ...
    if verbose:
        for ii in range(npks):
            ...
            print("T Loc: %.3f +/- %.3f" % (bparams[ii, 1], bunc[ii,1]))
            print("F Loc: %.2f +/- %.2f" % (bparams[ii, 2], bunc[ii,2]))
            print("T Wid: %.3f +/- %.3f" % (bparams[ii, 3], bunc[ii,3]))
