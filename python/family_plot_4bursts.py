...
def plot(ds, extent, tsamp, n_chan, tele, plot_grid_idx, fig, width,
    ...
    if lbls is not None:
        fs=8
        lbl0=ax1.text(0.10,0.92,(r'${\rm {\bf %s}}$')%lbls[0], ha='center', va='center', fontsize=fs, transform = ax1.transAxes,
                      bbox=dict(facecolor='white', alpha=0.9, edgecolor='black'))
        lbl1=ax2.text(0.10,0.65,(r'${\rm {\bf %s}}$')%lbls[1], ha='center', va='center', fontsize=fs, transform = ax2.transAxes,
                      bbox=dict(facecolor='grey', alpha=0.2, edgecolor='black'))
        lbl2=ax3.text(0.70,0.92,(r'${\rm {\bf %s}}$')%lbls[2], ha='center', va='center', fontsize=fs, transform = ax3.transAxes,
                      bbox=dict(facecolor='grey', alpha=0.2, edgecolor='black'))
...
def prep_data(dataframe, mode='sfxc'):
    ...
    ##filter the four bursts for the paper
    #O8: b08-o8
    #Stk: b06-stk
    #Tr: b08-stk
    #wb: B18-wb

    ids_4burst = ['B06-st', 'B08-tr', 'B13-o8', 'B18-wb']
    lbls = [['a', 'e', 'i'], ['b', 'f', 'j'], ['c', 'g', 'k'], ['d', 'h', 'l']]
    data_4burst = data[data['id'].isin(ids_4burst)]
    print(data_4burst)
    return ids_4burst, data_4burst, lbls
...
if __name__ == '__main__':
    ...
    ids, data, lbls = prep_data('../dbs/burst_info.csv')
    idx = 0
    part = 0
    nplots = 0
    ...
    #For perfect A4, standing
    #fig = plt.figure(figsize=[10,14.14])

    #For perfect A4, laying (chance nrows and ncols)
    #fig = plt.figure(figsize=[14.14,10])

    for burst, lbl in zip(ids, lbls):
        #if not burst == 'B07-o8':
        #    continue
        print("--------------------")
        print(f'{burst}')
        ...
        if (idx == nrows * ncols) or (nplots == len(ids)):
            ...
            #.055
            fig.subplots_adjust(hspace=0.8, wspace=0.5, left=0.07,right=.98,bottom=.15,top=.99)
            fig.savefig(f"../plots/burst_family_4burst_pt{part}.pdf", format = 'pdf', dpi=150)
            fig.savefig(f"../plots/burst_family_4burst_pt{part}.png", facecolor='white', transparent=False,format = 'png')
            try:
                fig.savefig(f"{os.environ['HOME']}/git/overleaf/r67-single-dish/figures/burst_family_4burst_pt{part}.pdf",
                            format = 'pdf', dpi=150)
                fig.savefig(f"{os.environ['HOME']}/git/overleaf/r67-single-dish/figures/burst_family_4burst_pt{part}.png",
